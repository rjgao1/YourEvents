(function() {
  ...
  function ajax(method, url, data, successCallback, errorCallback) {
    // All web asynchronous requests (send and receive) revolve around XMLHttpRequest
    var xhr = new XMLHttpRequest();

    xhr.open(method, url, true);

    // Asynchronous operation: xhr.responseText is set as the response -> you don't get the response through returned values 
    // onload is equivalent to onReadyStateChange when readyState is 4
    // handles when response is received (success or error)
    xhr.onload = function() {
      if (xhr.status === 200) {
    ...
    // Handles errors when request is being sent: e.g. network unavailable, server not ready etc
